...
const FundTransfer = () => {
  ...
  const TransferFormik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      return;
      BackendAxios.post(`/api/money-transfer`, values)
        .then((res) => {
          Toast({
            status: "success",
  ...
  const verifyBeneficiary = (queriedUserId) => {
    ...
    BackendAxios.get(
      `/api/users/${queriedUserId || TransferFormik.values.beneficiaryId}`
    )
      .then((res) => {
        setFetchedUser({
          ...fetchedUser,
  ...
  function fetchTransfers(pageLink) {
    BackendAxios.get(pageLink || 
      `/api/admin/user-reports/${transactionKeyword}/${Cookies.get("viewUserId")}?page=1`
      )
      .then((res) => {
        setPagination({
      ...
      <DashboardWrapper pageTitle={"Fund Transfer"}>
        ...
        <Box py={6}>
          <Box rounded={16} overflow={"hidden"}>
            <Box bg={"orange.500"} p={3} color={"white"}>
            ...
            <Box p={4}>
              ...
              <HStack justifyContent={"flex-end"}>
                <Button type="submit" onClick={()=>{
                  Toast({description: "Action not allowed."})
                }} colorScheme={"orange"}>
                  Submit
                </Button>
              </HStack>

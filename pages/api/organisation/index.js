...
export default async function GlobalDetails(req, res) {
    await Connect()
    if (req.method == "GET") {
        const result = await Organisation.findOne({ code: process.env.NEXT_PUBLIC_ORGANISATION })
        if (!result) return res.status(404).json({ message: "Organisation Info not found!" })
        return res.status(200).json(result)
    }

    if (req.method == "POST") {
        const result = await Organisation.findOneAndUpdate({code: process.env.NEXT_PUBLIC_ORGANISATION}, req.body).exec()
